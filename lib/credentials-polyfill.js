...
CredentialContainer.prototype.get = function(options) {
  ...
  if(!('identity' in query)) {
    if('password' in query || 'federated' in query) {
  ...
  // TODO: add `op=get` and `route=params` to URL
  ...
  return Flow.start(agentUrl, 'get', {query: query.identity});
};
...
CredentialContainer.prototype.store = function(credential, options) {
  ...
  // TODO: add `op=store` and `route=params` to URL
  var agentUrl = options.agentUrl ||
...
CredentialOperation.prototype.complete = function(result, options) {
  // TODO: add `op` via `operation.name` and `route=result` to URL
  var agentUrl = options.agentUrl ||
    'https://authorization.io/credentials?action=transmit';
  agentUrl = _updateQueryStringParameter(
    agentUrl, 'origin', window.location.origin);
...
IdentityCredential.register = function(options) {
  ...
  // TODO: add `op=registerDid` and `route=params` to URL
