...
IdentityCredential.register = function(options) {
  ...
  opts = {};
  if('agentUrl' in options) {
    opts.agentUrl = options.agentUrl;
  }
  return registration.register(opts);
};
...
local.IdentityCredentialRegistration = IdentityCredentialRegistration;
...
IdentityCredentialRegistration.prototype.addEventListener = function(
  event, listener) {
  return this._emitter.addEventListener(event, listener);
};
...
IdentityCredentialRegistration.prototype.removeEventListener = function(
  event, listener) {
  return this._emitter.removeEventListener(event, listener);
};
...
IdentityCredentialRegistration.prototype.register = function(options) {
  ...
  return Flow.start(agentUrl, 'registerDid', request, this._emitter);
};
...
(function() {
  ...
  try {
    style.sheet.insertRule(
      'dialog.credentials-polyfill-dialog::backdrop {background: transparent}',
      0);
  } catch(e) {}
})();
...
Flow.start = function(url, op, params, emitter) {
  ...
  // when context closes, always attempt to abort channel, which will
  // be a no-op if the channel is already properly shutdown
  context.onclose = abortIfNecessary;

  // abort flow if context is closed prematurely by detecting that
  // the window has gained focus (meaning the context has lost it entirely)
  window.addEventListener('focus', abortIfNecessary);
  function abortIfNecessary() {
    cleanup();
    channel.abort();
  }
  function cleanup() {
    window.removeEventListener('focus', abortIfNecessary);
  }
 ...
 *
 * For example, this call will send the result of the remote operation to the
...
/**
 * Called by the Credential Agent to emit an event during a remote API
 * operation.
 *
 * This call will send the event to the `opener browsing context`. The
 * `opener browsing context` is expected to then send a `continue` message
 * with an optional response message.
 *
 * @param name the name of the event.
 * @param event the event data.
 */
Router.prototype.emit = function(name, event) {
...
function BrowsingContext(url, options) {
  ...
  function showIframe() {
    ...
    dialog.className = 'credentials-polyfill-dialog';
    ...
    self.show = function() {
      if(!shown) {
        dialog.style.visibility = iframe.style.visibility = 'visible';
      }
      shown = true;
    };
  ...
  return new Promise(function(resolve, reject) {
    ...
    function listener(e) {
      ...
      if(e.source === self.end && e.origin === self.origin) {
        ...
        self._abort = null;
        self.context.show();
        // TODO: this validation could use some clean up, could be more
        // robust, and potentially be broken into separate functions
        ...
        if(typeof e.data === 'object' && 'data' in e.data) {
          ...
          if(e.data.type === 'event' &&
            ...
            if(e.data.data.name !== 'registerIdentityCredential') {
              return reject(
                new Error('Credential protocol error; unknown event.'));
            }
            // emit event, add message listener again, and trigger continue
            return self.emit(
              new RegisterIdentityCredentialEvent(e.data.data.event.publicKey))
              .catch(function(err) {
                // listen for channel message and send `error`
                window.addEventListener('message', listener);
                self.send('error', {name: err.name, message: err.message});
                return new Error('Error in event handler.');
              }).then(function(response) {
                if(!(response instanceof Error)) {
                  // listen for channel message and send `continue`
                  window.addEventListener('message', listener);
                  self.send('continue', response || null);
                }
              }).catch(function(err) {
                // failure to `send`, reject to handle error
                window.removeEventListener('message', listener);
                reject(err);
              });
          }
...
EventEmitter.prototype.addEventListener = function(event, listener) {
  ...
  if(event !== 'registerIdentityCredential') {
...
EventEmitter.prototype.emit = function(event) {
  ...
  return Promise.resolve(event._waitUntil);
};

/**
 * Creates a new RegisterIdentityCredentialEvent.
 *
 * @param publicKey the publicKey associated with the device that the
 *          IdentityCredential is to be registered on.
 *
 * @return the event to pass to emit.
 */
function RegisterIdentityCredentialEvent(publicKey) {
  var self = this;
  self.publicKey = publicKey;
  // causes event handling to pause until the Promise resolves with an
  // appropriate response based on the type of event
  self.respondWith = function(promise) {
    if(self._waitUntil) {
      throw new Error('`respondWith` already called.');
    }
    self._waitUntil = Promise.resolve(promise);
  };
}
