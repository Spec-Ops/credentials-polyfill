...
// FIXME: remove me
api._test = function(url) {
  return Flow.start(url, '_test', {options: {}});
};
...
function Router(type) {
  ...
  this.type = type;
  this.channel = new Channel(new BrowsingContext(window.opener));
}
...
Router.prototype.request = function(type) {
  return this.channel.request(type + '.' + this.type);
  // TODO: return more than just params, include identifier for
  // `this.channel.end`, `this.channel.origin` and any other information
  // that the agent would need to store
};
...
function BrowsingContext(url, options) {
  ...
  if(typeof url !== 'string') {
    ...
    self.handle = document.createElement('iframe');
    self.handle.style.display = 'none';
    self.handle.src = url;
    document.body.appendChild(self.handle);
    self.close = function() {
