 ...
 *
 * TODO: Refactor to request an IdentityCredential and receive it
 *
...
CredentialContainer.prototype.get = function(options) {
  ...
  return Flow.start(agentUrl, 'get', {query: options.query});
};

/**
 * Stores a Credential in the container.
 *
 * TODO: Refactor to store an IdentityCredential
 *
...
CredentialContainer.prototype.store = function(credential, options) {
  ...
  return Flow.start(agentUrl, 'store', credential);
};
...
/**
 * FIXME: There's an issue where an IdP could read `window.opener.location`
 * and learn a credential consumer's origin. To prevent this, the flow may
 * need to start by creating an iframe to the Credential Agent which will
 * cache the `window.opener` property and then set it to `null`. Then the
 * new window will be opened by the Credential Agent iframe instead of by
 * the consumer site. This breaks the opener chain between the consumer
 * and an IdP, however, there may still be a link via `window.top` that needs
 * to be addressed. Are there any other links that need to be broken? Are
 * some of them automatically broken when the pages reside on different origins?
 *
 ...
 *  - credentials.getCredentialRequest/credentials.getStorageRequest
 *    ===NEW IFRAME TO CREDENTIAL AGENT URL===
 *    - send `params` to opener (postMessage)
 *    ===CLOSE IFRAME===
 *  - create `result`
 *  - CredentialRequest/CredentialStorageRequest.resolve
 *    ===NEW IFRAME TO CREDENTIAL AGENT URL===
 ...
 * @param url the Credential Agent URL to use.
 *
 * @return a Promise that resolves to the type of flow and its parameters.
 */
...
Router.prototype.request = function(type) {
  var self = this;
  return self.channel.request(type + '.' + self.subject).then(function(data) {
    return {
...
Router.prototype.send = function(type, data) {
  this.channel.send(type + '.' + this.subject, data);
};
